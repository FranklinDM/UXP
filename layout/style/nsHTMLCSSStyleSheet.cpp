...
nsHTMLCSSStyleSheet::~nsHTMLCSSStyleSheet()
{
  ...
  for (auto iter = mCachedStyleAttrs.Iter(); !iter.Done(); iter.Next()) {
    ...
    if (value->mType == nsAttrValue::eCSSDeclaration) {
      Declaration* declaration = value->mValue.mCSSDeclaration;
                                          ...
                                          nsRuleWalker* aRuleWalker)
{
  ...
  Declaration* declaration = aElement->GetInlineStyleDeclaration();
  if (declaration) {
                                                ...
                                                nsRuleWalker* aRuleWalker)
{
  ...
  // just get the one and only style rule from the content's STYLE attribute
  Declaration* declaration = aPseudoElement->GetInlineStyleDeclaration();
  if (declaration) {
    declaration->SetImmutable();
    aRuleWalker->Forward(declaration);
  }
