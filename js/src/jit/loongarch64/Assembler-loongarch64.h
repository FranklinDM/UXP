...
#ifndef jit_loongarch64_Assembler_loongarch64_h
#define jit_loongarch64_Assembler_loongarch64_h
...
static constexpr Register WasmTlsReg = InstanceReg;
...
class AssemblerLOONGARCH64 : public AssemblerShared {
 ...
 public:
...
#if defined(__loongarch_hard_float) || defined(JS_SIMULATOR_LOONGARCH64)
    return true;
#else
    return false;
#endif
  ...
  static bool HasRoundInstruction(int mode) { (void)mode; return false; }
...
class InstReg : public Instruction {
 public:
  ...
  InstReg(OpcodeField op, AssemblerLOONGARCH64::FPConditionBit ca, FloatRegister fk,
  ...
  InstReg(OpcodeField op, FloatRegister fj, AssemblerLOONGARCH64::FPConditionBit cd)
      : Instruction(op | FJ(fj) | cd) {
    MOZ_ASSERT(op == op_movfr2cf);
  }
  InstReg(OpcodeField op, AssemblerLOONGARCH64::FPConditionBit cj, FloatRegister fd)
      : Instruction(op | (cj << CJShift) | FD(fd)) {
    MOZ_ASSERT(op == op_movcf2fr);
  }
  InstReg(OpcodeField op, Register rj, AssemblerLOONGARCH64::FPConditionBit cd)
      : Instruction(op | RJ(rj) | cd) {
    MOZ_ASSERT(op == op_movgr2cf);
  }
  InstReg(OpcodeField op, AssemblerLOONGARCH64::FPConditionBit cj, Register rd)
  ...
  InstReg(OpcodeField op, int32_t cond, FloatRegister fk, FloatRegister fj,
          AssemblerLOONGARCH64::FPConditionBit cd)
...
class InstImm : public Instruction {
 public:
  ...
  InstImm(OpcodeField op, int32_t si21, AssemblerLOONGARCH64::FPConditionBit cj,
          bool isNotEqual)
...
class Assembler : public AssemblerLOONGARCH64 {
 public:
...
#endif /* jit_loongarch64_Assembler_loongarch64_h */
