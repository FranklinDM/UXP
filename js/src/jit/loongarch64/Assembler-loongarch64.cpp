...
void AssemblerLOONGARCH64::finish() {
  MOZ_ASSERT(!isFinished);
  isFinished = true;
}

bool AssemblerLOONGARCH64::appendRawCode(const uint8_t* code, size_t numBytes) {
  return m_buffer.appendRawCode(code, numBytes);
}

bool AssemblerLOONGARCH64::reserve(size_t size) {
  // This buffer uses fixed-size chunks so there's no point in reserving
  // now vs. on-demand.
  return !oom();
}

bool AssemblerLOONGARCH64::swapBuffer(wasm::Bytes& bytes) {
...
void AssemblerLOONGARCH64::copyJumpRelocationTable(uint8_t* dest) {
  if (jumpRelocations_.length()) {
    memcpy(dest, jumpRelocations_.buffer(), jumpRelocations_.length());
  }
}

void AssemblerLOONGARCH64::copyDataRelocationTable(uint8_t* dest) {
  if (dataRelocations_.length()) {
    memcpy(dest, dataRelocations_.buffer(), dataRelocations_.length());
  }
}

void AssemblerLOONGARCH64::copyPreBarrierTable(uint8_t* dest) {
  if (preBarriers_.length()) {
    memcpy(dest, preBarriers_.buffer(), preBarriers_.length());
  }
}

AssemblerLOONGARCH64::Condition AssemblerLOONGARCH64::InvertCondition(Condition cond) {
  switch (cond) {
    case Equal:
      return NotEqual;
    case NotEqual:
...
AssemblerLOONGARCH64::DoubleCondition AssemblerLOONGARCH64::InvertCondition(
...
AssemblerLOONGARCH64::Condition AssemblerLOONGARCH64::InvertCmpCondition(
    Condition cond) {
  switch (cond) {
    case Equal:
    case NotEqual:
...
bool AssemblerLOONGARCH64::oom() const {
  return AssemblerShared::oom() || m_buffer.oom() || jumpRelocations_.oom() ||
         dataRelocations_.oom() || preBarriers_.oom();
}

// Size of the instruction stream, in bytes.
size_t AssemblerLOONGARCH64::size() const { return m_buffer.size(); }

// Size of the relocation table, in bytes.
size_t AssemblerLOONGARCH64::jumpRelocationTableBytes() const {
  return jumpRelocations_.length();
}

size_t AssemblerLOONGARCH64::dataRelocationTableBytes() const {
  return dataRelocations_.length();
}

size_t AssemblerLOONGARCH64::preBarrierTableBytes() const {
  return preBarriers_.length();
}

// Size of the data table, in bytes.
size_t AssemblerLOONGARCH64::bytesNeeded() const {
...
void AssemblerLOONGARCH64::WriteInstStatic(uint32_t x, uint32_t* dest) {
...
// Logical operations.
BufferOffset AssemblerLOONGARCH64::as_and(Register rd, Register rj, Register rk) {
  spew("and    %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_and, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_or(Register rd, Register rj, Register rk) {
  spew("or     %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_or, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_xor(Register rd, Register rj, Register rk) {
  spew("xor    %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_xor, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_nor(Register rd, Register rj, Register rk) {
  spew("nor    %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_nor, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_andn(Register rd, Register rj, Register rk) {
  spew("andn    %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_andn, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_orn(Register rd, Register rj, Register rk) {
  spew("orn     %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_orn, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_andi(Register rd, Register rj, int32_t ui12) {
...
BufferOffset AssemblerLOONGARCH64::as_ori(Register rd, Register rj, int32_t ui12) {
  MOZ_ASSERT(is_uintN(ui12, 12));
  spew("ori    %3s,%3s,0x%x", rd.name(), rj.name(), ui12);
  return writeInst(InstImm(op_ori, ui12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_xori(Register rd, Register rj, int32_t ui12) {
  MOZ_ASSERT(is_uintN(ui12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_b(JOffImm26 off) {
  spew("b    %d", off.decode());
  return writeInst(InstJump(op_b, off).encode());
}

BufferOffset AssemblerLOONGARCH64::as_bl(JOffImm26 off) {
  spew("bl    %d", off.decode());
  return writeInst(InstJump(op_bl, off).encode());
}

BufferOffset AssemblerLOONGARCH64::as_jirl(Register rd, Register rj,
                                       BOffImm16 off) {
  spew("jirl   %3s, %3s, %d", rd.name(), rj.name(), off.decode());
  return writeInst(InstImm(op_jirl, off, rj, rd).encode());
}

InstImm AssemblerLOONGARCH64::getBranchCode(JumpOrCall jumpOrCall) {
  // jirl or beq
...
InstImm AssemblerLOONGARCH64::getBranchCode(FPConditionBit cj) {
  return InstImm(op_bcz, 0, cj, true);  // bcnez
}

// Arithmetic instructions
BufferOffset AssemblerLOONGARCH64::as_add_w(Register rd, Register rj, Register rk) {
  spew("add_w   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_add_w, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_add_d(Register rd, Register rj, Register rk) {
  spew("add_d   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_add_d, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_sub_w(Register rd, Register rj, Register rk) {
  spew("sub_w   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_sub_w, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_sub_d(Register rd, Register rj, Register rk) {
  spew("sub_d   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_sub_d, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_addi_w(Register rd, Register rj,
                                         int32_t si12) {
...
BufferOffset AssemblerLOONGARCH64::as_addi_d(Register rd, Register rj,
                                         int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_addu16i_d(Register rd, Register rj,
                                            int32_t si16) {
...
BufferOffset AssemblerLOONGARCH64::as_alsl_w(Register rd, Register rj, Register rk,
                                         uint32_t sa2) {
  MOZ_ASSERT(sa2 < 4);
...
BufferOffset AssemblerLOONGARCH64::as_alsl_wu(Register rd, Register rj, Register rk,
                                          uint32_t sa2) {
  MOZ_ASSERT(sa2 < 4);
...
BufferOffset AssemblerLOONGARCH64::as_alsl_d(Register rd, Register rj, Register rk,
                                         uint32_t sa2) {
  MOZ_ASSERT(sa2 < 4);
...
BufferOffset AssemblerLOONGARCH64::as_lu12i_w(Register rd, int32_t si20) {
  spew("lu12i_w   %3s,0x%x", rd.name(), si20);
  return writeInst(InstImm(op_lu12i_w, si20, rd, false).encode());
}

BufferOffset AssemblerLOONGARCH64::as_lu32i_d(Register rd, int32_t si20) {
  spew("lu32i_d   %3s,0x%x", rd.name(), si20);
  return writeInst(InstImm(op_lu32i_d, si20, rd, false).encode());
}

BufferOffset AssemblerLOONGARCH64::as_lu52i_d(Register rd, Register rj,
                                          int32_t si12) {
...
BufferOffset AssemblerLOONGARCH64::as_slt(Register rd, Register rj, Register rk) {
  spew("slt   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_slt, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_sltu(Register rd, Register rj, Register rk) {
  spew("sltu   %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_sltu, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_slti(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("slti   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_slti, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_sltui(Register rd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_srli_w(Register rd, Register rj,
                                         int32_t ui5) {
  MOZ_ASSERT(is_uintN(ui5, 5));
...
BufferOffset AssemblerLOONGARCH64::as_srai_w(Register rd, Register rj,
                                         int32_t ui5) {
  MOZ_ASSERT(is_uintN(ui5, 5));
...
BufferOffset AssemblerLOONGARCH64::as_rotri_w(Register rd, Register rj,
                                          int32_t ui5) {
  MOZ_ASSERT(is_uintN(ui5, 5));
...
BufferOffset AssemblerLOONGARCH64::as_srli_d(Register rd, Register rj,
                                         int32_t ui6) {
  MOZ_ASSERT(is_uintN(ui6, 6));
...
BufferOffset AssemblerLOONGARCH64::as_srai_d(Register rd, Register rj,
                                         int32_t ui6) {
  MOZ_ASSERT(is_uintN(ui6, 6));
...
BufferOffset AssemblerLOONGARCH64::as_rotri_d(Register rd, Register rj,
                                          int32_t ui6) {
  MOZ_ASSERT(is_uintN(ui6, 6));
...
BufferOffset AssemblerLOONGARCH64::as_bytepick_d(Register rd, Register rj,
...
BufferOffset AssemblerLOONGARCH64::as_bstrins_d(Register rd, Register rj,
...
BufferOffset AssemblerLOONGARCH64::as_bstrpick_w(Register rd, Register rj,
                                             int32_t msbw, int32_t lsbw) {
  MOZ_ASSERT(lsbw <= msbw);
...
BufferOffset AssemblerLOONGARCH64::as_bstrpick_d(Register rd, Register rj,
                                             int32_t msbd, int32_t lsbd) {
  MOZ_ASSERT(lsbd <= msbd);
...
BufferOffset AssemblerLOONGARCH64::as_maskeqz(Register rd, Register rj,
                                          Register rk) {
  spew("maskeqz    %3s,%3s,%3s", rd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_maskeqz, rk, rj, rd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_masknez(Register rd, Register rj,
                                          Register rk) {
...
BufferOffset AssemblerLOONGARCH64::as_ld_b(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("ld_b   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_ld_b, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_ld_h(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("ld_h   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_ld_h, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_ld_w(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("ld_w   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_ld_w, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_ld_d(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("ld_d   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_ld_d, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_ld_bu(Register rd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_ld_hu(Register rd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_ld_wu(Register rd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_st_b(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("st_b   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_st_b, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_st_h(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("st_h   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_st_h, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_st_w(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
  spew("st_w   %3s,%3s,0x%x", rd.name(), rj.name(), si12);
  return writeInst(InstImm(op_st_w, si12, rj, rd, 12).encode());
}

BufferOffset AssemblerLOONGARCH64::as_st_d(Register rd, Register rj, int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_vseq_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vseq.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vseq_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsle_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsle.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vsle_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsle_wu(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsle.wu  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vsle_wu, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vslt_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vslt.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vslt_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vslt_wu(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vslt.wu  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vslt_wu, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vadd_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vadd.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vadd_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsub_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsub.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vsub_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vmax_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vmax.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vmax_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vmin_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vmin.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vmin_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vmax_wu(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vmax.wu  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vmax_wu, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vmin_wu(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vmin.wu  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vmin_wu, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vmul_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vmul.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vmul_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsll_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsll.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vsll_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsrl_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsrl.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vsrl_w, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vsra_w(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vsra.w   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
...
BufferOffset AssemblerLOONGARCH64::as_vand_v(FloatRegister vd,
  ...
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vand.v   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vand_v, vk, vj, vd).encode());
}
...
BufferOffset AssemblerLOONGARCH64::as_vnor_v(FloatRegister vd,
                                             FloatRegister vj,
                                             FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vnor.v   %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vnor_v, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vandn_v(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vandn.v  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vandn_v, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vfadd_s(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vfadd.s  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vfadd_s, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vfsub_s(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vfsub.s  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vfsub_s, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vfmul_s(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vfmul.s  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vfmul_s, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vfmax_s(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vfmax.s  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vfmax_s, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vfmin_s(FloatRegister vd,
                                              FloatRegister vj,
                                              FloatRegister vk) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(vk.isSimd128());
  spew("vfmin.s  %3s,%3s,%3s", vd.name(), vj.name(), vk.name());
  return writeInst(InstReg(op_vfmin_s, vk, vj, vd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_vneg_w(FloatRegister vd,
                                             FloatRegister vj) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
...
BufferOffset AssemblerLOONGARCH64::as_vslli_w(FloatRegister vd,
                                              FloatRegister vj,
                                              uint32_t imm) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(imm < 32);
  spew("vslli.w  %3s,%3s,%u", vd.name(), vj.name(), imm);
  return writeInst(op_vslli_w | (imm << RKShift) | FJ(vj) | FD(vd));
}

BufferOffset AssemblerLOONGARCH64::as_vsrli_w(FloatRegister vd,
                                              FloatRegister vj,
                                              uint32_t imm) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(imm < 32);
  spew("vsrli.w  %3s,%3s,%u", vd.name(), vj.name(), imm);
  return writeInst(op_vsrli_w | (imm << RKShift) | FJ(vj) | FD(vd));
}

BufferOffset AssemblerLOONGARCH64::as_vsrai_w(FloatRegister vd,
                                              FloatRegister vj,
                                              uint32_t imm) {
  MOZ_ASSERT(vd.isSimd128());
  MOZ_ASSERT(vj.isSimd128());
  MOZ_ASSERT(imm < 32);
  spew("vsrai.w  %3s,%3s,%u", vd.name(), vj.name(), imm);
  return writeInst(op_vsrai_w | (imm << RKShift) | FJ(vj) | FD(vd));
}
...
BufferOffset AssemblerLOONGARCH64::as_dbar(int32_t hint) {
  MOZ_ASSERT(is_uintN(hint, 15));
  spew("dbar   0x%x", hint);
  return writeInst(InstImm(op_dbar, hint).encode());
}

BufferOffset AssemblerLOONGARCH64::as_ibar(int32_t hint) {
  MOZ_ASSERT(is_uintN(hint, 15));
  spew("ibar   0x%x", hint);
...
BufferOffset AssemblerLOONGARCH64::as_fmadd_d(FloatRegister fd, FloatRegister fj,
                                          FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fmsub_s(FloatRegister fd, FloatRegister fj,
                                          FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fmsub_d(FloatRegister fd, FloatRegister fj,
                                          FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fnmadd_s(FloatRegister fd, FloatRegister fj,
                                           FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fnmadd_d(FloatRegister fd, FloatRegister fj,
                                           FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fnmsub_s(FloatRegister fd, FloatRegister fj,
                                           FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fnmsub_d(FloatRegister fd, FloatRegister fj,
                                           FloatRegister fk, FloatRegister fa) {
...
BufferOffset AssemblerLOONGARCH64::as_fcopysign_d(FloatRegister fd,
                                              FloatRegister fj,
                                              FloatRegister fk) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cor(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_ceq(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cne(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cle(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_clt(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cun(FloatFormat fmt, FloatRegister fj,
                                           FloatRegister fk,
                                           FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cueq(FloatFormat fmt, FloatRegister fj,
                                            FloatRegister fk,
                                            FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cune(FloatFormat fmt, FloatRegister fj,
                                            FloatRegister fk,
                                            FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cule(FloatFormat fmt, FloatRegister fj,
                                            FloatRegister fk,
                                            FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fcmp_cult(FloatFormat fmt, FloatRegister fj,
                                            FloatRegister fk,
                                            FPConditionBit cd) {
  if (fmt == DoubleFloat) {
...
BufferOffset AssemblerLOONGARCH64::as_fld_d(FloatRegister fd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_fst_s(FloatRegister fd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_fst_d(FloatRegister fd, Register rj,
                                        int32_t si12) {
  MOZ_ASSERT(is_intN(si12, 12));
...
BufferOffset AssemblerLOONGARCH64::as_fldx_s(FloatRegister fd, Register rj,
                                         Register rk) {
  spew("fldx_s    %3s,%3s,%3s", fd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_fldx_s, rk, rj, fd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_fldx_d(FloatRegister fd, Register rj,
                                         Register rk) {
  spew("fldx_d    %3s,%3s,%3s", fd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_fldx_d, rk, rj, fd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_fstx_s(FloatRegister fd, Register rj,
                                         Register rk) {
  spew("fstx_s    %3s,%3s,%3s", fd.name(), rj.name(), rk.name());
  return writeInst(InstReg(op_fstx_s, rk, rj, fd).encode());
}

BufferOffset AssemblerLOONGARCH64::as_fstx_d(FloatRegister fd, Register rj,
                                         Register rk) {
...
void AssemblerLOONGARCH64::bind(Label* label, BufferOffset boff) {
...
void AssemblerLOONGARCH64::retarget(Label* label, Label* target) {
...
void AssemblerLOONGARCH64::as_break(uint32_t code) {
...
void AssemblerLOONGARCH64::PatchWrite_Imm32(CodeLocationLabel label, Imm32 imm) {
...
uint8_t* AssemblerLOONGARCH64::NextInstruction(uint8_t* inst_, uint32_t* count) {
...
void AssemblerLOONGARCH64::ToggleToJmp(CodeLocationLabel inst_) {
...
void AssemblerLOONGARCH64::ToggleToCmp(CodeLocationLabel inst_) {
  InstImm* inst = (InstImm*)inst_.raw();
...
InstImm AssemblerLOONGARCH64::invertBranch(InstImm branch, BOffImm16 skipOffset) {
  uint32_t rj = 0;
...
#ifdef JS_JITSPEW
void AssemblerLOONGARCH64::decodeBranchInstAndSpew(InstImm branch) {
  OpcodeField opcode = (OpcodeField)((branch.extractBitField(31, 26)) << 26);
  uint32_t rd_id;
  uint32_t rj_id;
  uint32_t cj_id;
