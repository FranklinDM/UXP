...
GenerateReturn(MacroAssembler& masm, int returnCode)
{
    ...
    masm.loadPtr(Address(StackPointer, offsetof(EnterJITRegs, s8)), s8);
...
GeneratePrologue(MacroAssembler& masm)
{
    ...
    masm.storePtr(s8, Address(StackPointer, offsetof(EnterJITRegs, s8)));
 ...
 * CodeGeneratorLOONGARCH64::generateOutOfLineCode()) and thunk code should save it
 * on stack. Other difference is that members snapshotOffset_ and padding_ are
 * pushed to the stack by CodeGeneratorLOONGARCH64::visitOutOfLineBailout(). Field
...
PushBailoutFrame(MacroAssembler& masm, Register spArg)
{
    ...
    // See: CodeGeneratorLOONGARCH64::generateOutOfLineCode()
    masm.push(ra);
...
{
    MOZ_CRASH("LOONGARCH64 does not use bailout tables");
}
    ...
    for (uint32_t explicitArg = 0; explicitArg < f.explicitArgs; explicitArg++) {
        ...
        switch (f.argProperties(explicitArg)) {
          ...
          case VMFunction::DoubleByRef:
            MOZ_CRASH("NYI: LOONGARCH64 callVM should not be used with 128bits values.");
            break;
        }
    }
